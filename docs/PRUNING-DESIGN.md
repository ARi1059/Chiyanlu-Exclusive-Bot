...
> | --- | --- | --- |
> | P1 设计文档 | ✅ 已完成 | 本文档 |
> | **P2 dry-run** | **✅ 已完成** | [scripts/prune.sh](../scripts/prune.sh) `--dry-run` 落地；白名单 2 张表（`user_events` / `user_teacher_views`）；任何 `--confirm / --delete / --vacuum / --execute` 立即 exit 1；缺 `--dry-run` 也 exit 1 |
> | P3 实际清理（--confirm） | ⬜ **未启动**，**当前 scripts/prune.sh 不具备删除能力** | 任何带 `--confirm` 的调用都会被拒绝 |
> | P4 healthcheck DB 体积提醒 | ⬜ 未启动 | |
> | P5 RUNBOOK pruning 操作流程 | 🟡 部分完成 | 本次已加 dry-run 用法指引；P3 落地后再补 confirm 流程 |
> | P6 scheduler 接入 | ⬜ 未启动，权益类**永不**接入 | |
>
> **当前阶段（P2）严格只读**：脚本对数据库的所有调用仅包含 `PRAGMA integrity_check`
> / `PRAGMA journal_mode` / `PRAGMA table_info` / `SELECT`，**没有任何 DELETE /
> UPDATE / INSERT / VACUUM / DROP / ALTER / REINDEX 语句**。线上**仍然没有**任何
> 真实清理逻辑；本文档下文中所有以"建议"、"未来"措辞的小节，仍是 P3+ 阶段的目标。
...
> 当前版本**只**支持 `--dry-run`。任何 `--confirm / --delete / --vacuum / --execute`
> 参数都会立即 `[ERR ]` 并 exit 1，缺 `--dry-run` 也会 exit 1，防止误启动 P3 路径。
...
| **P3** | `scripts/prune.sh --confirm`，仅清理 P2 白名单内的表；强制先 backup | ⬜ 未启动 | 低 | 行数下降，业务不可感知 |
