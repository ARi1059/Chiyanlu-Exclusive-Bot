...
| `reimbursements_new` 非空 | **不要删表**，按 docs/POLICY-reimbursement.md 人工对比后由开发者处理 |
...
> 政策原文见 [POLICY-lottery.md](POLICY-lottery.md)。
...
> 政策原文见 [POLICY-reimbursement.md](POLICY-reimbursement.md)。
...
1. **对照 POLICY-reimbursement.md** 判断是否符合策略，不能凭印象。
...
# 报销池余量（按你们配置的池模型字段查，名称参考 POLICY-reimbursement.md）
...
> 政策原文见 [POLICY-points.md](POLICY-points.md)。
...
1. **对照 POLICY-points.md** 确认获取 / 消耗规则。
