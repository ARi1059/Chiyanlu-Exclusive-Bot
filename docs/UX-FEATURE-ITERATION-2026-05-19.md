...
| 7 | UX-7.2 报销名单批量激活 | **暂不开放**（2026-05-20 与用户对齐补充） | 涉及资金流动；误操作会让用户进入审批队列后被打款；当前单条逐审能力满足 |

### 11.2 触发重评估的条件

| 决策 | 重评估条件 |
|---|---|
| UX-7.2（报销批量） | 运营报告 queued 名单常规 ≥ 50 条积压；并配套"批量 dry-run 预览页"+ batch_id 回滚机制设计审议通过 |
| UX-7.3（评价批量） | UX-7.2 报销批量上线后再评估（现 UX-7.2 已暂不开放，本项触发条件同步延后） |
...
Week 1:    UX-10.1（安全加固，首个 PR）+ UX-4.6（校验节奏）
           UX-5.6 已由 commit:0c6df9d 提前落地，跳过
Week 2:    UX-4.1 ~ UX-4.5（5 个通知 CTA）
...
- [x] UX-10.1 已实现 commit:5c88696（2026-05-19）
- [x] UX-4.6 已实现 commit:b7942a1（2026-05-20）
- [x] UX-5.6 已实现 commit:0c6df9d（2026-05-19，PLAN/ROADMAP 立项的看板命名拆分先于本计划完成）
- [x] UX-4.1 已实现 commit:9856958（2026-05-20）
- [x] UX-4.2 已实现 commit:538fb18（2026-05-20）
- [x] UX-4.3 已实现 commit:2964ba5（2026-05-20）
- [x] UX-4.4 已实现 commit:365d7eb（2026-05-20）
- [x] UX-4.5 已实现 commit:753f64e（2026-05-20）
- [x] UX-5.1 已实现 commit:d6383af（2026-05-20）
- [x] UX-5.2 已实现 commit:525387d（2026-05-20）
- [x] UX-5.3 已实现 commit:5bbc3c9（2026-05-20）
- [x] UX-5.4 已实现 commit:35b18d5（2026-05-20）
- [x] UX-5.5 已实现 commit:8e5f0f1（2026-05-20，简化版方案 C：仅时区标注，无周期 edit）
- [x] UX-6.1 已实现 commit:fe7ef44（2026-05-20）
...
- **§7.2 痛点 1**（"📈 数据分析"vs"📊 运营看板"命名易混）：commit `0c6df9d`（2026-05-19 refactor）已把 `dashboard:enter` 从「📊 数据看板」→「📈 数据分析」、`admin:dashboard` 从「📊 数据看板」→「📊 运营看板」。语义已足够区分，**痛点关闭**。
- **§7.3 建议 1**（数据分析/运营看板命名拆分）：原立项已落地，本计划 §7.3 中"进一步改为「历史分析」/「用户行为」"属基于 explore agent 的主观加码建议，**不进入本轮执行**；如生产观察发现管理员仍混淆，未来 Sprint 再评估。
