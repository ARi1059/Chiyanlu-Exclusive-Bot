...
| 前端技术栈 | ✅ 已定（2026-06-26 改定 React） | **React 18 + Vite + TS + Tailwind + shadcn/radix + recharts + react-hook-form**（基线 = Figma Make 生成的 `Design Mini App Frontend/`） |
...
前端统一用 `MediaImage.tsx` 组件：拿到的就是可直接 `src` 的 URL（代理 URL 或存储 URL），组件无需关心来源。**「媒体代理 + 缓存」是 P1 第一个落地件**——后面所有富展示（相册轮播、评价手势照、审核媒体组）都依赖它。
...
> 2026-06-26 改定：原推荐 Vue 3，但已用 Figma Make 基于本方案生成 React 原型（`Design Mini App Frontend/`），定为前端基线；后端 API 框架无关，不受影响。

| 关注点 | 选型 | 说明 |
|---|---|---|
| 框架 | **React 18 + TypeScript + Vite 6** | Figma Make 产出基线；TG MiniApp 模板成熟、生态最大 |
| UI | **Tailwind 4 + shadcn/ui**（radix-ui + cva + clsx + tailwind-merge） | 原型已用；无样式约束、组件可控 |
| 图表 | **recharts** | 原型已用：6 维雷达图、看板面积/趋势图 |
| 表单 | **react-hook-form** | 写评价 / 档案录入多字段 |
| 富交互 | **embla / react-slick**（相册轮播）· **react-dnd**（档案拖拽）· **react-responsive-masonry**（评价瀑布流）· **vaul**（drawer）· **sonner**（toast） | 原型已装，精准对应方案功能 |
| 路由 | **react-router 7** + 角色守卫 | 一套工程按 session 角色分 user/teacher/admin |
| TG 集成 | **@twa-dev/sdk**（待加） | initData / themeParams / BackButton / MainButton；原型尚未接入 |
| 状态 / 请求 | React hooks（按需 zustand）· fetch/axios + 拦截器 | 自动带 session，401 静默换 token |

原型现状：单文件 `src/app/App.tsx` 交互原型 + mock 数据，`Role` / `Teacher` / `Review` 结构已与后端契约基本对齐（字段名 `name` / `available` ↔ `display_name` / `is_today_available` 待统一）。
...
### 8.2 前端：以 `Design Mini App Frontend/`（Figma Make 导出的 React 工程）为基线

```
Design Mini App Frontend/src/
  tg.ts            # SDK 封装：取 initData、套 theme、接管 BackButton/MainButton（待加）
  api/             # fetch/axios + 拦截器（自动带 session、401 静默换 token）（待加）
  app/             # react-router 角色守卫 user/teacher/admin（原型现为单文件 App.tsx，待拆）
  components/  MediaImage.tsx  TeacherCard.tsx  ReviewForm.tsx（从 App.tsx 拆出）
  imports/MINIAPP-MIGRATION.md   # 本方案（Figma Make 的输入）
```
...
```
（`album` 内即 `MediaImage.tsx` 可直接 `src` 的 URL）
...
**交付物**：`POST /api/uploads`（file_id 回灌）、`GET /api/teachers/{id}/review-context`、`POST /api/reviews`、收藏/提醒三端点、`PATCH /api/teacher/profile`；前端 `ReviewForm.tsx`、收藏页写交互、老师端资料表单。
...
把现有文本指标升级为 recharts，数据零改、复用现有 service：
...
**交付物**：审核台三 Tab（claim/force-claim/release + approve/reject/payout）、档案录入分区表单 + 相册 + 预览发布、四块看板。前端 `admin/` 视图集（shadcn 表格/抽屉/表单 + recharts）。
