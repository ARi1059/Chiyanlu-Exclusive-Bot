...
- 后台主菜单按钮「⚙️ 系统配置」入口（超管 / 管理员都可见，子项按权限差异化展示），收纳：
  - 📢 必关订阅 → `admin:subreq`（handler 在 `subreq_admin.py`）
  - 🧩 发布模板 → `admin:publish_templates`（handler 在 `publish_templates.py`）
  - 📣 频道 / 群组设置 → `menu:channel`（handler 在 `admin_panel.py`）
  - 📅 日报 / 周报设置 → `admin:report_settings`（handler 在 `report_settings.py`）
  - ⚙️ 系统设置 → `menu:system`（含发布时间 / 冷却时间 / 签到提醒 / 品牌等深层项）
  - **仅超管** 💰 报销池设置 → `system:reimburse_pool`
  - **仅超管** 🔛 报销功能开关 → `system:reimburse_toggle`
- 关键词管理无独立 callback（群关键词为 message handler），本菜单不构造伪入口
- 仅菜单结构调整：七个 callback 含义、handler、权限边界全部保持不变；老消息中的旧 inline button 仍可使用
- 实现位于 [`bot/handlers/admin_panel.py`](bot/handlers/admin_panel.py) 的 `cb_admin_settings` + [`bot/keyboards/admin_kb.py`](bot/keyboards/admin_kb.py) 的 `admin_settings_kb()`
...
- 后台主菜单按钮「🛡 管理员设置」入口（**仅超管可见**），收纳：
  - 👥 管理员管理 → `menu:admin`（@super_admin_required，子菜单含 添加 / 移除 / 列表）
  - 📜 审计日志 → `dashboard:audit`（@admin_required，最近 20 条 admin_audit_logs）
- 主菜单 Row 1 行为差异：超管 = `[👩‍🏫 老师管理, 🛡 管理员设置]`；非超管 = `[👩‍🏫 老师管理]` solo（隐藏管理员设置入口，同时修复 pre-existing UX bug —— 之前 `menu:admin` 按钮对非超管也展示但 handler 拒绝点击）
- 仅菜单结构调整：四个 callback 含义、handler、`@super_admin_required` 权限边界全部保持不变；`menu:admin` 旧入口仍可用（旧消息中的 inline button 仍可点击）
- 主菜单精简至最终形态：super 4 行 / 非 super 3 行
- 实现位于 [`bot/handlers/admin_panel.py`](bot/handlers/admin_panel.py) 的 `cb_admin_admin_settings` + [`bot/keyboards/admin_kb.py`](bot/keyboards/admin_kb.py) 的 `admin_admin_settings_kb()`
...
scripts/backup.sh / scripts/prune.sh` 四类检查。workflow 定义见
