...
普通用户**首次** `/start` 看到 3 按钮的新手引导（今日开课 / 直接搜索 / 进入主菜单），点击任意一个后标记"已看过"，下次进入直接显示主菜单。
...
**说明**：Phase A0（2026-05-23）已下线 🎁 抽奖中心 / 📜 搜索历史 / 🕘 最近看过 / 📝 我的记录聚合；A0 后（2026-06-13）又下线 🎯 帮我推荐 / 🔎 按条件找 / 🔥 热门推荐（详见 [docs/DELETED-FEATURES.md](docs/DELETED-FEATURES.md)）。
...
**操作**：聚合保留的查找入口（A0 后热门 / 筛选已下线）；按钮点击进入对应子模块。
...
### 1.4 ~~🎯 帮我推荐 / 🔎 按条件找 / 🔥 热门推荐~~（已下线）

> A0 后（2026-06-13）下线这三项面向用户的找老师功能，群内对应关键词自动回复同步移除。
> 现统一走 `[📚 今天能约谁]`（今日开课）与 `[🔍 直接搜索]`。
> 后台「🔥 热门推荐管理」（`admin:hot_manage`，推荐位 `is_featured`，仍是全站排序基础设施）保留。
> 详见 [docs/DELETED-FEATURES.md](docs/DELETED-FEATURES.md)。
...
**停用 vs 删除**（2026-06-13 新增）：
- **停用**（`is_active=0`）：暂时下架，仍显示在管理列表、可一键启用恢复；适合临时不接单。
- **删除**（软删除 `is_deleted=1`）：从用户端和管理端所有列表彻底隐藏，但评价 / 收藏 / 报销等历史数据保留；可由**超管**在 `[♻️ 恢复老师]` 找回（恢复后回到删除前的启用 / 停用态）。
...
│   │   ├── teacher_flow.py              # 老师启停 / 软删除恢复 / 简版 CRUD
│   │   ├── user_panel.py                # 用户主菜单 + 找老师聚合
│   │   ├── user_search.py               # 直接搜索 FSM
│   │   ├── (已下线) user_filter / user_recommend  # 热门 / 推荐 / 筛选，见 docs/DELETED-FEATURES.md
